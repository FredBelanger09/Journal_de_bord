...
Où $b in BeauB$ sont les cas de bases (en particuliers les constances $c in BeauC$ ), $alpha in nu$ les variables de types, $BigZero$ le type vide et $BigOne$ le supertype de tout les types. $BeauB$ est composé de $Int$ (l'ensemble des entiers relatifs) et Enum (l'ensemble des types énumérés, par exemple bool).
...
Contrairement à ce que voudrait la logique, les atomes de la forme $and.big (t_1 -> t_2) and.big not(t_1 -> t_2)$ sont ici considérés comme des cas de base, car toutes les fonctions sont habitées, et connaitre uniquement un cas d'utilisation serait complexe et improductif. on considère donc que $a ::= and.big (t_1 -> t_2) and.big not(t_1 -> t_2)$ est un cas de base.

On propose donc comme type témoin :
$ w ::= c | a | w times w $

En pratique, on a $c ::= i in [|Int|] | e in [|Enum|]$, ainsi que des extensions pour les tags, les n-uplets et les records, ce qui donne :
...
- Ici, $f$ est une extension de notre type t qui inclus les types optionnels (ex : $f = Int?$). On le défini comme $t in BigOne union bot$, $bot$ dénotant le type absent.
- Le bindings est un champ où l'on place toutes les variables nommées de notre record (ex : ${x : Int; y : Int?; "predicat" : Bool}$).
- La tail $f$ indique si notre record est ouvert ou fermé (donc s'il peut avoir des variables supplémentaires non nommées). (ex : ${x : Int;" " y : Int;;" " Int?}$ pour un système de coordonnées à au moins 2 dimensions)
- Pour chaque paire $(L_i : e_i)$ de exists, $L_i$ indique les noms que la variables ne peut PAS prendre et $e_i and f$ indique son type (ex : ${x : Int; y : Int;; Any?;; {w;z} : Bool?}$ pour indiquer que les variables $w$ et $z$ ne peuvent pas avoir le type $Bool$ si elles existent)
...
#proof[
  ...
  $<=> t = or.big ((and.big_i (p_1^i, p_2^i))\\ or.big_j (n_1^j, n_2^j))$

  $<=> t = or.big ((and_i p_1^i,and_i p_2^i)\\ or.big_j (n_1^j, n_2^j))$ par commutativité de $and$
  ...
  On a maintenant $or.big_i (t_1,t_2)$ une union, montrons que $or.big_i (t_1^i,t_2^i) \\ (n_1,n_2)$ est une union :
  $or.big_i (t_1^i,t_2^i) \\ (n_1,n_2) = or.big_i ((t_1^i,t_2^i) \\ (n_1,n_2) )$  $= or.big_i ((t_1^i \\ n_1,t_2^i) or (t_1^i,t_2^i \\ n_2))$ qui est une union.
...
#proof(title: "Preuve ")[
  On peut déjà noter que pour tout type t, $Delta$ a une taille maximale finie, car il est au maximum de la taille l'ensemble de tout les sous-types de t (qui est fini car t est régulier). RENVOYER A LA DEF DE REGULIER
...
== Preuve de la justesse de l'algorithme que si $emptyset |-""_m t ~> w$ alors $w:t$
...
#proof(title: "Preuve ")[
  ...
  Montrons que $P(t_1 times t_2, w_1 times w_2)$ :\ Si $(t_1 times t_2) ~> (w_1 times w_2)$, alors par $times_t$, $t_1 ~> w_1$ et $t_2 ~> w_2$, donc par hypothèse, $w_1 :t_1$ et $w_2 :t_2$, donc par $times_w$, $(w_1 times w_2) : (t_1 times t_2)$.
